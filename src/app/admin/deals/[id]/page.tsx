...
const emptyFlight: FlightInfo = { airline: '', flightNumber: '', departureAirport: '', departureCity: '', arrivalAirport: '', arrivalCity: '', date: '', departureTime: '', arrivalTime: '', stops: [] };
const emptyHotel: HotelInfo = { name: '', stars: 4, roomType: '', roomDescription: '', boardBasis: '', checkIn: '', checkOut: '', images: [] };

const boardBasisOptions = [
  { value: '', label: 'לא צוין' }, { value: 'RO', label: 'לינה בלבד (RO)' }, { value: 'BB', label: 'לינה + ארוחת בוקר (BB)' },
  { value: 'HB', label: 'חצי פנסיון (HB)' }, { value: 'FB', label: 'פנסיון מלא (FB)' }, { value: 'AI', label: 'הכל כלול (AI)' },
];
const tagColors = [
  { value: 'bg-primary-500', label: 'כחול' }, { value: 'bg-accent-500', label: 'כתום' }, { value: 'bg-green-500', label: 'ירוק' },
  { value: 'bg-rose-500', label: 'ורוד' }, { value: 'bg-amber-500', label: 'זהב' }, { value: 'bg-violet-500', label: 'סגול' },
];
const currencies = [
...
export default function EditDealPage() {
  ...
  const [formData, setFormData] = useState({
    title: '', titleEn: '', destination: '', destinationEn: '', country: '', countryEn: '',
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (Array.isArray(cats)) setCategories(cats.filter((c: any) => c.isActive !== false));
      if (deal && !deal.error) {
        setFormData({
          title: deal.title || '', titleEn: deal.titleEn || '',
          destination: deal.destination || '', destinationEn: deal.destinationEn || '',
          country: deal.country || '', countryEn: deal.countryEn || '',
          description: deal.description || '', descriptionEn: deal.descriptionEn || '',
          price: deal.price?.toString() || '', originalPrice: deal.originalPrice?.toString() || '',
          currency: deal.currency || 'ILS', nights: deal.nights?.toString() || '1',
          image: deal.image || '', images: deal.images || [],
          tag: deal.tag || '', tagEn: deal.tagEn || '', tagColor: deal.tagColor || 'bg-primary-500',
          categoryId: deal.categoryId?.toString() || '', spotsLeft: deal.spotsLeft?.toString() || '',
        ...
        if (deal.outboundFlight) setOutbound(deal.outboundFlight);
        if (deal.returnFlight) setReturnFlight(deal.returnFlight);
        else setNoReturn(!deal.returnFlight && !!deal.outboundFlight);
        if (deal.hotel) setHotel(deal.hotel);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/deals/${id}`, {
        ...
        body: JSON.stringify({
          ...formData,
          price: formData.price, originalPrice: formData.originalPrice || null,
          nights: parseInt(formData.nights), categoryId: formData.categoryId || null,
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-slate-700 mb-2">Deal Name (English)</label><input type="text" value={formData.titleEn} onChange={e => setFormData({ ...formData, titleEn: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" dir="ltr" /></div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div><label className="block text-sm font-medium text-slate-700 mb-2">תיאור (עברית)</label><textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" /></div>
                <div><label className="block text-sm font-medium text-slate-700 mb-2">Description (English)</label><textarea value={formData.descriptionEn} onChange={e => setFormData({ ...formData, descriptionEn: e.target.value })} rows={3} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" dir="ltr" /></div>
              </div>
              <div className="grid md:grid-cols-3 gap-4">
                <div><label className="block text-sm font-medium text-slate-700 mb-2">קטגוריה</label><select value={formData.categoryId} onChange={e => setFormData({ ...formData, categoryId: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500"><option value="">ללא קטגוריה</option>{categories.map(c => <option key={c.id} value={c.id.toString()}>{c.title}</option>)}</select></div>
                <div><label className="block text-sm font-medium text-slate-700 mb-2">תגית</label><input type="text" value={formData.tag} onChange={e => setFormData({ ...formData, tag: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
                <div><label className="block text-sm font-medium text-slate-700 mb-2">צבע תגית</label><div className="flex gap-2 pt-1">{tagColors.map(c => <button key={c.value} type="button" onClick={() => setFormData({ ...formData, tagColor: c.value })} className={`w-8 h-8 rounded-lg ${c.value} ${formData.tagColor === c.value ? 'ring-2 ring-offset-2 ring-slate-400' : ''}`} title={c.label} />)}</div></div>
              </div>
              <div className="flex gap-6">
                <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={formData.isActive} onChange={e => setFormData({ ...formData, isActive: e.target.checked })} className="w-5 h-5 text-primary-600 rounded" /><span className="text-slate-700">פעיל</span></label>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4">
              ...
              {formData.includes.map((item, i) => (
                <div key={i} className="flex gap-2">
                  <input type="text" value={item} onChange={e => { const n = [...formData.includes]; n[i] = e.target.value; setFormData({ ...formData, includes: n }); }} className="flex-1 px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" />
              ...
              <button type="button" onClick={() => setFormData({ ...formData, includes: [...formData.includes, ''] })} className="flex items-center gap-2 text-primary-600 hover:text-primary-700 font-medium"><Plus className="w-5 h-5" /> הוסף פריט</button>
            </div>
          </div>
        )}

        {activeSection === 'outbound' && <FlightSection title="טיסה הלוך" flight={outbound} onChange={setOutbound} icon="✈️" />}
        ...
        {activeSection === 'return' && (
          <div className="space-y-4">
            ...
            {!noReturn && <FlightSection title="טיסה חזור" flight={returnFlight} onChange={setReturnFlight} icon="🔄" />}
          </div>
        )}
        ...
        {activeSection === 'pricing' && (
          <div className="bg-white rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div><label className="block text-sm font-medium text-slate-700 mb-2">מחיר *</label><input type="number" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" required /></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-2">מחיר מקורי</label><input type="number" value={formData.originalPrice} onChange={e => setFormData({ ...formData, originalPrice: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-2">מספר לילות</label><input type="number" value={formData.nights} onChange={e => setFormData({ ...formData, nights: e.target.value })} min="1" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
            </div>
            <div><label className="block text-sm font-medium text-slate-700 mb-2">מקומות שנותרו</label><input type="number" value={formData.spotsLeft} onChange={e => setFormData({ ...formData, spotsLeft: e.target.value })} min="0" placeholder="ריק = ללא הגבלה" className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 max-w-xs" /></div>
            <div className="border-t pt-6 space-y-4">
              <h3 className="text-md font-semibold text-slate-800">מדיניות ביטול</h3>
              <label className="flex items-center gap-3 cursor-pointer" onClick={() => setFormData({ ...formData, freeCancellation: !formData.freeCancellation })}>
                <div className={`relative w-12 h-7 rounded-full transition-colors ${formData.freeCancellation ? 'bg-green-500' : 'bg-slate-300'}`}><div className={`absolute top-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${formData.freeCancellation ? 'translate-x-5' : 'translate-x-0.5'}`} /></div>
...
function FlightSection({ title, flight, onChange, icon }: { title: string; flight: FlightInfo; onChange: (f: FlightInfo) => void; icon: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div><label className="block text-sm font-medium text-slate-700 mb-2">תאריך</label><input type="date" value={flight.date} onChange={e => update('date', e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
        <div><label className="block text-sm font-medium text-slate-700 mb-2">שעת המראה</label><input type="time" value={flight.departureTime} onChange={e => update('departureTime', e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" dir="ltr" /></div>
        <div><label className="block text-sm font-medium text-slate-700 mb-2">שעת נחיתה</label><input type="time" value={flight.arrivalTime} onChange={e => update('arrivalTime', e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" dir="ltr" /></div>
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div><label className="block text-sm font-medium text-slate-700 mb-2">דירוג כוכבים</label><div className="flex gap-1 pt-1">{[1, 2, 3, 4, 5].map(n => <button key={n} type="button" onClick={() => update('stars', n)} className="p-1"><Star className={`w-7 h-7 transition-colors ${n <= hotel.stars ? 'text-amber-400 fill-amber-400' : 'text-slate-300'}`} /></button>)}</div></div>
      </div>
      <div className="grid md:grid-cols-2 gap-4">
        <div><label className="block text-sm font-medium text-slate-700 mb-2">סוג חדר</label><input type="text" value={hotel.roomType} onChange={e => update('roomType', e.target.value)} placeholder="Standard / Superior / Suite..." className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
        <div><label className="block text-sm font-medium text-slate-700 mb-2">בסיס אירוח</label><select value={hotel.boardBasis} onChange={e => update('boardBasis', e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500">{boardBasisOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>
      </div>
      <div><label className="block text-sm font-medium text-slate-700 mb-2">תיאור החדר</label><textarea value={hotel.roomDescription} onChange={e => update('roomDescription', e.target.value)} placeholder="חדר מרווח עם מרפסת ונוף לים..." rows={3} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none" /></div>
      <div className="grid md:grid-cols-2 gap-4">
        <div><label className="block text-sm font-medium text-slate-700 mb-2">צ׳ק-אין</label><input type="date" value={hotel.checkIn} onChange={e => update('checkIn', e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
        <div><label className="block text-sm font-medium text-slate-700 mb-2">צ׳ק-אאוט</label><input type="date" value={hotel.checkOut} onChange={e => update('checkOut', e.target.value)} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500" /></div>
      </div>
      <div className="border-t pt-4 space-y-3">
...
function MultiImageUploader({ images, onChange }: { images: string[]; onChange: (imgs: string[]) => void }) {
  ...
  const handleFiles = async (files: FileList) => {
    ...
    onChange(newImages); setUploading(false);
  };
  return (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {images.map((url, i) => (
          <div key={i} className="relative group aspect-video rounded-xl overflow-hidden border border-slate-200">
            ...
            <button type="button" onClick={() => onChange(images.filter((_, idx) => idx !== i))} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-4 h-4" /></button>
          </div>
        ))}
